...
# ranks to extract
ranks = [ 'superkingdom', 'kingdom', 'clade', 'phylum', 'class', 'order', 'family',
         'genus', 'species']
    ...
    while True:
        ...
        if rank in rank_dic:
            ...
            if rank_tids[tid][rankx] != '0':
                print('taxon %s has duplicated definition of rank %s. Old name is %s. New name is %s.'
                      % (tid, rankx, rank_names[tid][rankx], taxdump[cid]['name']))
            ...
            if taxdump[cid]['name'] not in ['Patescibacteria group','Bacteria candidate phyla']:
